...
SELECT PRODUCT from sales GROUP BY product_id HAVING COUNT(DISTINCT region_id) = (SELECT COUNT(*) FROM regiond)

-- 3.	Retrieve the name of the manager who supervises the most employees.

SELECT manager_id , COUNT(*) as num_emp from employees GROUP BY  manager_id ORDER BY num_emp desc limit 1
